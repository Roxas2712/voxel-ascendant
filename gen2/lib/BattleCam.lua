...
function BattleCam.stepZoom(notches, arena)
  ...
  -- The limits describe the original rig's frameH. Searched MAPs already
  -- multiply that frame by 1.6/2.2; applying the limits a second time made
  -- even an initial zoom-IN jump OUT from the authored starting value 1.
  local scale = arena and arena.mapReframe
    and math.max(1, tonumber(arena.mapFrameScale) or 1) or 1
  BattleCam.zoomGoal = math.max(BattleCam.ZOOM_MIN / scale,
                        math.min(BattleCam.ZOOM_MAX / scale,
...
function BattleCam.frameH(arena)
  local base = BattleCam.rigFor(arena).frameH
    * math.max(1, tonumber(arena and arena.mapFrameScale) or 1)
...
-- A searched Gold MAP keeps its validated camera seat still, but its lens
-- must consume manual pinch/wheel input even when the cinematic director
-- declines that shot. Do not advance orbit, pitch or idle drift in this path.
function BattleCam.updateZoom(dt)
...
function BattleCam.rig(arena, groundY, canonical)
  ...
  return {
    eye = eye,
    focus = focus,
    -- Vertical FOV is the angle from the lower edge to the upper edge: twice
    -- the half-angle formed by frameH/2. Gen 2 had accidentally dropped this
    -- factor while Kanto kept the solved equation, halving both authored
    -- lenses and magnifying portable-stage Pokemon far beyond the viewport on
    -- the send-out/menu hand-off.
