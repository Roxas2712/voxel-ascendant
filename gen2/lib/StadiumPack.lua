...
-- A Gen-1 Stadium-2 import must not overwrite or accept a historical
-- Stadium-1 cache which happens to use the same 001..151 filenames.
if V.stadium2ForGen1 == true then
  StadiumPack.CACHE_DIR = "cache/stadium2-gen1"
  StadiumPack.DIR = "assets/stadium2-gen1"
else
...
-- The model for a National Dex number. Gen2Recomped can import the complete
-- Stadium 2 roster (1..251); Gen 1 hosts still naturally stop at 151 because
-- no higher species ids are requested.
function StadiumPack.load(species)
  if not (species and species >= 1 and species <= 251) then return nil end
