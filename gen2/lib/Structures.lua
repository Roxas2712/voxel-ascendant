...
-- Structure analysis follows the same bounded apron as ChunkMesher. More
-- importantly, the first/current BODY build analyses the body only: the old
-- fixed 32-tile scan carved thousands of synthetic border trees before one
-- playable tile could be shown, even though BODY never emits that apron.
local function configuredRingTiles()
  local profile = "handheld"
  if DeviceProfile and type(DeviceProfile.effective) == "function" then
    local ok, value = pcall(DeviceProfile.effective)
    if ok and type(value) == "string" then profile = value end
  end
  return math.max(0, (tonumber(PerformancePolicy.apronBlocks(profile)) or 3) * 4)
end

function Structures.analysisRing(bodyOnly)
  return bodyOnly == true and 0 or configuredRingTiles()
end
...
-- map id -> { map, def, blocks, tileset, body, full, fullRing }. BODY and
-- FULL deliberately keep separate analyses: promoting a ready body must not
-- expose partially carved apron objects, while the full ring may continue in
-- the background. Source receipts also keep a newly materialised map object
-- with the same id from inheriting stale geometry.
local cache = {}

local function cacheRecord(map)
  local c = cache[map.id]
  local def = map.def
  local blocks = def and def.blocks
  if not c or c.def ~= def or c.blocks ~= blocks or c.tileset ~= map.tileset then
    c = { map = map, def = def, blocks = blocks, tileset = map.tileset }
    cache[map.id] = c
  else
    -- A direct-neighbour adapter and Gold's promoted current Map are distinct
    -- Lua objects over the same immutable def/blocks/tileset. Keep the warmed
    -- analysis across that honest promotion while retaining the source tuple
    -- above as its stale-data guard.
    c.map = map
  end
  return c
end
...
local function indoorShell(map, shapes)
  local def = map.def
  if Map.isOutdoor(def) then return nil end
  local counts, quads = {}, {}
  for cx = 0, def.width * 2 - 1 do
    local tx = cx * 2
    -- judged on the cell's SW tile, the one collision is read from
    local s = TileShape.at(map, shapes, map:tileAt(tx, 1), tx, 1)
...
function Structures.runHeight(map, tx, ty)
  local c = cache[map.id]
  local S = c and c.def == map.def
    and c.blocks == (map.def and map.def.blocks)
    and c.tileset == map.tileset and (c.body or c.full) or nil
  local run = S and S.runs[keyOf(tx, ty)]
  return run and run.h or nil
end

-- `releaseAux` deliberately keeps the compact height runs after a completed
-- upload, but removes every construction table.  A later slot refresh or a
-- freshly materialised map root may legitimately request geometry again.
-- Returning that compact runtime receipt from forMap made ChunkMesher index a
-- missing shapeAt table and drop the new map back to native 2D.  Reuse only a
-- complete construction analysis; otherwise rebuild this slot in isolation.
local function constructionReady(S)
  return type(S) == "table"
    and type(S.shapeAt) == "table"
    and type(S.tileAt) == "table"
    and type(S.skip) == "table"
    and type(S.ground) == "table"
    and type(S.objectQuads) == "table"
    and type(S.roundStamps) == "table"
    and type(S.grassQuads) == "table"
    and type(S.flowerQuads) == "table"
...
function Structures.forMap(map, bodyOnly)
  ...
  local S = bodyOnly and c.body
    or (c.fullRing == ring and c.full or nil)
  if constructionReady(S) then return S end
  if S then
    if bodyOnly then
      c.body = nil
    else
  ...
  Budget.phase("structure:tile-shapes")
  ...
  --
  -- GOLD / GEN 2: current Gold map defs do not identify outdoor maps with
  -- the old Gen-1 literal `OVERWORLD` tileset name.  They expose the map
  -- environment (TOWN / ROUTE), which Map.isOutdoor(def) resolves.  The old
  -- name-only gate therefore missed Gold completely: its synthetic tree
  -- surround ran the full meshing ring and, outside ROUND_RING where the
  -- expensive hull pass intentionally stops, fell back to ordinary upright
  -- boxes.  From a tilted camera those boxes are the giant tree-textured
  -- walls seen around towns and routes.
  --
  -- For Gold, do not assume EVERY outdoor border is forest: ocean routes
  -- and special maps are allowed to carry a different borderBlock.  Confirm
  -- that this block actually uses one of this tileset profile's round-tree
  -- classes before applying the forest-ring treatment.
  -- The profile already names the tiles that belong to round tree art.
  -- Resolve that set once so both the synthetic Gold border and the real
  -- map body can use the same conservative test.  This is intentionally
  -- profile-driven rather than a hardcoded Johto tile list: Forest, Park
  -- and future Gold outdoor tilesets get the same protection when they
  -- declare cylinder / planter / canopy art.
  Budget.phase("structure:tree-profile")
  local goldTreeTiles = nil
  if tileset.collision and Map.isOutdoor(def) then
  ...
  local function goldBorderUsesTreeArt()
    if not (borderBlk and goldTreeTiles) then return false end
    local hits = 0
    for _, t in ipairs(borderBlk) do
      if goldTreeTiles[t] then hits = hits + 1 end
    end
    -- One hit can be an incidental lone tree in an otherwise unrelated
    -- border block; two or more means the repeated border itself is using
    -- the tree drawing.
    return hits >= 2
  end
  ...
  -- actual border block because current Gold does not route that option
  -- through the old OVERWORLD literal.
  local treeVoidFill = (TileRenderer.voidFill or "trees") == "trees"
  local goldTreeBorder = goldBorderUsesTreeArt()
  local hullRingOnly = borderBlk
                       and ((def.tileset == "OVERWORLD" and treeVoidFill)
                            or goldTreeBorder)
  ...
  local function tileLookup(tx, ty)
    ...
    local inRoundRing = not (tx < -ring or ty < -ring
                         or tx >= tw2 + ring
                         or ty >= th2 + ring)
  ...
  Budget.phase("structure:resolve-grid")
    ...
    for tx = x0, x1 do
      ...
      if tile then
        ...
        if inShell(tx, ty) then
          ...
          -- GOLD TREE-BORDER FAILSAFE.  A Gold outdoor border block is
          -- filler scenery, not a building.  Normally its tree tiles are
          -- already authored as `cylinder` / `planter`, but an engine or
          -- extracted-cache variation can leave one of those cells on the
          -- generic `upright` fallback.  Never let that fallback become a
          -- textured rectangular wall outside the real map body: carve the
          -- same 16x16 source art with the round tree hull instead.
          --
          -- Only unauthored UPRIGHT cells outside the body are changed.
          ...
          if outsideBody and Map.isOutdoor(def) and s and not s.flat
            ...
            -- back into a rectangular wall. Even an authored WALL/CLIFF class
            -- here is still just the same repeated borderBlock tree artwork.
            -- Force the near belt into one-cell round stamps; the far belt is
            -- already clipped by hullRingOnly where the profile identifies it.
  ...
  -- GOLD IN-MAP TREE FAILSAFE.  The normal profile pins are still the
  -- authoritative path: Johto's tall tree wall resolves as a two-cell
  -- `planter`, lone trees as `cylinder`, and Forest/Park can use `canopy`.
  -- But a changed/extracted Gold blockset can leave a tree cell on the
  -- generic blocked-cell `upright` fallback even though the actual four
  -- 8x8 graphics are overwhelmingly tiles that this profile names as round
  -- tree art.  That is the in-map version of the screenshot artefact: one
  -- rectangular prism wearing a tree texture.
  --
  -- Promote ONLY those unresolved 16x16 cells, ONLY on a Gold outdoor map,
  -- and only when at least half of the cell's four source tiles are known
  -- round-tree art.  Authored shapes are never touched, so proper two-cell
  -- planters/canopies keep their richer grouping, while buildings, cliffs,
  -- signs and fences cannot be caught merely because they are solid.
  Budget.phase("structure:tree-failsafe")
  if goldTreeTiles then
          ...
          if hits >= 2 then
            for dy = 0, 1 do
              for dx = 0, 1 do
                ...
                local cs = shapeAt[ck]
                -- A mixed cell may contain an authored ground/edge tile.
                -- Do not erase that explicit pin; only replace fallback
                -- shapes that would otherwise become a box.
  ...
  -- Gen 1 maps expose a `doorTiles` lookup table keyed by cell tile.
  -- Gen 2 maps expose `Map:isDoorTileCell(cx, cy)` instead; indexing the
  -- absent Gen-1 table used to throw inside the async mesher and leave Gold
  -- permanently on the flat fallback with a false-cached voxel build.
  Budget.phase("structure:doors")
  local hasDoorMethod = type(map.isDoorTileCell) == "function"
  local doorTiles = type(map.doorTiles) == "table" and map.doorTiles or nil
  for cy = math.floor(y0 / 2), math.floor(y1 / 2) do
    for cx = math.floor(x0 / 2), math.floor(x1 / 2) do
      local isDoor = hasDoorMethod and map:isDoorTileCell(cx, cy)
        or (doorTiles and doorTiles[map:cellTile(cx, cy)])
      if isDoor then
  ...
  Budget.phase("structure:cylinders")
  ...
  Budget.phase("structure:bookcases")
  ...
  Budget.phase("structure:figures")
  Structures.buildFigures(S, map, x0, x1, y0, y1, bodyOnly)
  ...
  Budget.phase("structure:model-regions")
  local data = pixels(tileset)
  ...
  Budget.phase("structure:pinned-props")
  if data then
    local seenB = {}
    for ty = y0, y1 do
      for tx = x0, x1 do
    ...
    -- Auxiliary cards are shared with the BODY mesh and rendered once over
    -- the composed neighbourhood.  A FULL/apron analysis still performs all
    -- terrain classification below, but must not duplicate tens of thousands
    -- of retained grass quads that can never be drawn separately.
    Budget.phase("structure:grass")
    if bodyOnly then
      Structures.buildGrass(S, map, 0, tw - 1, 0, th - 1, data)
    end

    -- ---- flowers: the animated meadow tile stands as a 1px cutout ----
    Budget.phase("structure:flowers")
  ...
  -- happen to vote in.
  Budget.phase("structure:prop-ground")
  do
    local okP, prof = pcall(V.data, "voxel_heights")
    local entry = okP and type(prof) == "table" and prof.tilesets
                  and prof.tilesets[tileset.id]
  ...
  Budget.phase("structure:ground-votes")
...
local function roundTemplate(S, map, data, cx, cy, groundTiles, N, capRows,
  ...
  local cls = {}
  for py = 0, NY - 1 do
    -- A new tileset can require several previously unseen 16/32px hull
    -- templates in one BODY build.  Keep each template cooperative too;
    -- otherwise all of its pixel classification lands in the one frame that
    -- happened to encounter the first tree.
    Budget.check()
    for px = 0, NX - 1 do
  ...
  if johtoTree then
    ...
    for y = 0, NY - 1 do
      ...
      for x = 0, NX - 1 do
        local i = y * NX + x
        -- Gold tiles are opaque, but preserve transparent/off pixels for
        -- replacement atlases that intentionally punch holes in the tree.
...
function Structures.buildCylinders(S, map, x0, x1, y0, y1, groundTiles)
  ...
  for cy = math.floor(y0 / 2), math.floor(y1 / 2) do
    for cx = math.floor(x0 / 2), math.floor(x1 / 2) do
      ...
      local ring = tonumber(S.analysisRing) or 0
      local near = cx * 2 >= -ring and cx * 2 < tw + ring
               and cy * 2 >= -ring and cy * 2 < th + ring
      ...
      if not isRail(tx, ty) then
        ...
        for t = tx, txb do
          ...
          if px1 > px0 then
            ...
            quads[#quads + 1] = {
              { t * 8 + px0, railTop, za }, { t * 8 + px1, railTop, za },
              { t * 8 + px1, railTop, zb }, { t * 8 + px0, railTop, zb },
              shade = 1,
              uv = { { u0, v0 }, { u1, v0 }, { u1, v1 }, { u0, v1 } } }
          end
          local k = keyOf(t, ty)
...
local function buildFigure(S, map, fig, tx, ty, perRow, emitCard)
...
function Structures.buildFigures(S, map, x0, x1, y0, y1, emitCards)
  ...
  for _, fig in ipairs(figures) do
    for ty = y0, y1 - fig.h + 1 do
      for tx = x0, x1 - fig.w + 1 do
        ...
        if hit then
          buildFigure(S, map, fig, tx, ty, perRow, emitCards ~= false)
        end
      end
    end
  end
...
function Structures.buildFlowers(S, map, tw, th, x0, x1, y0, y1, data,
  ...
  local quads = emitQuads == false and nil or S.flowerQuads
  for ty = y0, y1 do
    for tx = x0, x1 do
      Budget.tick()
      local k = keyOf(tx, ty)
      ...
      if s and s.art == "flower" then
        ...
        if quads and tx >= 0 and ty >= 0 and tx < tw and ty < th then
          local tileId = S.tileAt[k]
          local tpl = templates[tileId]
          if not tpl then
...
-- Once ChunkMesher has copied an analysis into GPU buffers, the expanded
-- per-face Lua tables are dead data.  Keeping them beside the ready mesh made
-- a three-map Johto neighbourhood retain close to a gigabyte (each pixel face
-- is several nested Lua tables), which in turn caused paging stalls while the
-- next route appeared.  The measured height runs remain: entity placement is
-- the only runtime reader of a completed Structures analysis.
local function validRecord(map)
  local c = cache[map.id]
  if c and c.def == map.def and c.blocks == (map.def and map.def.blocks)
      and c.tileset == map.tileset then
    return c
  end
  return nil
end

function Structures.releaseGeometry(map, bodyOnly)
  local c = validRecord(map)
  local S = c and (bodyOnly and c.body or c.full) or nil
  if not S then return end
  S.objectQuads = nil
  S.roundStamps = nil
end

-- Release only the analysis whose mesh has actually finished.  BODY and FULL
-- are built by independent coroutines.  Clearing both records when the first
-- job landed let that job erase shapeAt underneath the other coroutine; its
-- next resume then failed in ChunkMesher.runGeometry and the renderer fell
-- back to 2D after a map transition.  A nil bodyOnly keeps the old all-records
-- behaviour for explicit maintenance callers, while normal mesher completion
-- passes the owning slot.
function Structures.releaseAux(map, bodyOnly)
  local c = validRecord(map)
  if not c then return end
  local records
  if bodyOnly == true then
    records = { c.body }
  elseif bodyOnly == false then
    records = { c.full }
  else
    records = { c.body, c.full }
  end
  for _, S in pairs(records) do
    if S then
      S.objectQuads = nil
      S.roundStamps = nil
      S.grassQuads = nil
