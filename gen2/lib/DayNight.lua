...
-- Existing stored strings stay valid. An unset Gen-2 option follows the
-- cartridge's own journey clock; FREE RUN retains the old `cycle` value.
DayNight.setting = ModSetting.new(DayNight.KEY, DayNight.LABEL,
                                  { "native", "cycle", "day", "night",
                                    "dusk", "dawn" },
                                  { "GAME", "FREE RUN", "DAY", "NIGHT",
                                    "DUSK", "DAWN" },
                                  "native")
...
function DayNight.forceDay(game)
  if DayNight.setting:get() ~= "day" then
    DayNight.setting:setValue("day", game)
  end
end

DayNight.clock = DayNight.T.day     -- the running cycle's own position
DayNight.nativeHour = 12
DayNight.nativeMinute = 0
DayNight.nativePeriod = "DAY"
...
local function mode()
  return DayNight.setting:get() or "native"
end
...
function DayNight.isNative()
  return mode() == "native"
end

function DayNight.nativeTime(hour, minute)
  hour = tonumber(hour)
  if hour == nil then return DayNight.T.day end
  ...
  if minutes >= 4 * 60 and minutes < 10 * 60 then
    return (minutes - 4 * 60) / (6 * 60) * DayNight.T.day
  end
  if minutes >= 10 * 60 and minutes < 18 * 60 then
    return DayNight.T.day
      + (minutes - 10 * 60) / (8 * 60)
        * (DayNight.T.dusk - DayNight.T.day)
  end
  if minutes < 4 * 60 then minutes = minutes + 24 * 60 end
  return DayNight.T.dusk
    + (minutes - 18 * 60) / (10 * 60)
      * (DayNight.CYCLE - DayNight.T.dusk)
end

local GEN2_PERIOD = {
  MORN="MORN", MORNING="MORN", DAY="DAY",
  NITE="NITE", NIGHT="NITE", EVENING="NITE", DARK="DARK",
}

function DayNight.syncNativeClock(hour, minute, period)
  local numericHour = tonumber(hour)
  if numericHour == nil then return false end
  DayNight.nativeHour = math.floor(numericHour) % 24
  local numericMinute = tonumber(minute)
  DayNight.nativeMinute = numericMinute ~= nil
    and math.max(0, math.min(59, math.floor(numericMinute))) or 0
  local canonical = GEN2_PERIOD[tostring(period or ""):upper()]
  if canonical then DayNight.nativePeriod = canonical end
  return true
end

-- The effective time: native RTC, a deterministic pin, or FREE RUN.
function DayNight.time()
  local m = mode()
  if m == "native" then
    return DayNight.nativeTime(DayNight.nativeHour, DayNight.nativeMinute)
  end
...
function DayNight.update(dt)
  ...
  if m ~= lastMode then
    if m == "cycle" then
      if lastMode == "native" then
        DayNight.clock = DayNight.nativeTime(
          DayNight.nativeHour, DayNight.nativeMinute)
      else
...
function DayNight.gen2Tod(t)
  if t == nil and DayNight.isNative() then
    return DayNight.nativePeriod or "DAY"
  end
  return GEN2_PERIOD[DayNight.tod(t)] or "DAY"
end
