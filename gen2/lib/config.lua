...
function Config.schema()
  -- The integrated dispatcher already composed native Gen2 + internal Card
  -- rows. Re-reading standalone options here silently erased all APO rows
  -- from the engine manager (the VASC hub still had its separate copy).
  if type(V.mod._vascGen2Schema)=="table" then
    return V.mod._vascGen2Schema
  end
