...
local function refreshEffectProfile(side)
  if not session or (side ~= "player" and side ~= "enemy") then return nil end
  local arena = session.arena
  local cell = arena and arena[side]
  if type(cell) ~= "table" then return nil end

  local mon = session[side]
  local hasModel = mon and mon.model ~= nil
  local fallbackHeight = side == "player" and 17 or 20
  local height = hasModel and mon:worldHeight() or fallbackHeight
  local radius = hasModel and mon:worldRadius() or height * 0.24
  local scale = hasModel and mon.scale or 1
  session.effectAnchors = session.effectAnchors or {}
  session.effectAnchors[side] = BattleEffectAnchors.profile(
    cell[1], cell[2], session.groundY or 0, height, radius, scale,
    hasModel and "stadium-model" or "arena-fallback")
  return session.effectAnchors[side]
end
...
function Stadium.mode()
  ...
  if type(OverworldBattle.mode) == "function" then
    local mode = OverworldBattle.mode()
    if mode ~= nil and (mode == OverworldBattle.MAP
        or mode == OverworldBattle.ARENA or mode == true) then
      return "A"
    end
    if mode ~= nil and (mode == OverworldBattle.DISCS
        or mode == OverworldBattle.FLAT_B) then return "B" end
    if mode == Stadium.VALUE then return "A" end
    if mode == Stadium.VALUE_B then return "B" end
    return nil
  end
  if OverworldBattle.enabled and OverworldBattle.enabled() then return "A" end
...
function Stadium.enabled()
  ...
  if type(V.modelsEnabled) == "function" then
    local ok, enabled = pcall(V.modelsEnabled)
...
function Stadium.begin(arena)
  ...
  session = {
    ...
    at = {},
    -- Gold's BattleState can keep the outgoing mon in shownMon after the
    -- logic battle has already rebound battle[side]. These latches belong to
    -- that presentation identity, not to the already-advanced logic slot.
    gen2Entrance = {},
    gen2Faint = {},
    effectAnchors = {},
  }
  -- Establish safe fallback profiles as soon as the battle map has selected
  -- its arena. They are refined from the real models when those become ready.
...
-- Narrow presentation accessor for the optional Stage-1 animation bridge.
-- Older builds found the private `session` local through debug.getupvalue;
-- release/sandbox Lua builds are allowed to omit that debug API, which made
-- the whole animation installer fail even though the two actors were live.
-- Expose only the requested actor, never the session table or battle rules.
function Stadium.stage1Actor(side)
  if side ~= "player" and side ~= "enemy" then return nil end
  return session and session[side] or nil
end
...
-- A 251-file cache may remain after KASC is disabled. The runtime boundary
-- therefore mirrors the importer boundary and declines every post-151 actor
-- in standalone Gen 1 instead of silently reusing extension data.
local function catalogDex(dex)
  ...
  if ok and type(install) == "table"
    ...
    local okLimit, value = pcall(install.targetCount)
    if okLimit and tonumber(value) then limit = tonumber(value) end
  end
  if dex < 1 or dex > limit then return nil end
  return dex
end

Stadium._catalogDex = catalogDex
...
local function gen2ActiveMon(screen, side)
  ...
  if screen and type(screen.activeMon) == "function" then
    ...
    if active ~= nil then battler = active end
  end
  return battler
end

local function gen2Substitute(screen, side, battler)
  local legacy = battler and battler.substituteHP
  if legacy == true or (tonumber(legacy) or 0) > 0 then return true end
  local volatile = battler and battler.volatile
  local substitute = volatile and volatile.substitute
  if substitute == true or (tonumber(substitute) or 0) > 0 then return true end
  local anim = screen and screen.anim
  local override = anim and anim.picOverride
  return type(override) == "table" and override[side] == "substitute"
end

local function gen2AnimPicState(screen, side)
  if not (screen and type(screen.animPicState) == "function") then return nil end
  local ok, state = pcall(screen.animPicState, screen, side)
  if not ok then
    error(("Crystal %s animation visibility failed: %s")
      :format(tostring(side), tostring(state)), 0)
  end
  return state
end

local function gen2SideVisible(screen, side, battler, mon)
  ...
  elseif screen.showEnemyTrainer then
    -- InitEnemyTrainer owns the enemy slot until trainer-slide completes.
    -- OverworldBattle captures that trainer portrait as the visible world
    -- card; the opponent model may appear only after the send-out boundary.
    return false
  end
  if gen2Substitute(screen, side, battler) then return false end
  local vanished = battler.volatile and battler.volatile.vanished
  if vanished and not (screen.vanishAnim and screen.vanishAnim == screen.anim) then
    return false
  end
  -- Once Gold starts MonFaintedAnimation, a real Stadium collapse owns the
  -- side until its held final frame completes. If this species has no usable
  -- collapse clip, decline and let the exact native faint slide through.
  local faintSlide = screen.faintSlide
  local fainting = type(faintSlide) == "table" and faintSlide.side == side
  if mon and mon.state == "faint" then
    local finished = type(mon.finished) == "function" and mon:finished()
    return not finished
  elseif fainting then
    return false
  end
  local anim = gen2AnimPicState(screen, side)
  if type(anim) == "table" and anim.hidden then return false end
  if type(hidden) == "table" and hidden[side] then return false end
  return true
end

Stadium._gen2ActiveMon = gen2ActiveMon
Stadium._gen2SideVisible = gen2SideVisible
...
function Stadium.updateGen2(dt, screen, groundY)
  ...
  for _, side in ipairs({ "enemy", "player" }) do
    local mon = session[side]
    local battler = gen2ActiveMon(screen, side)
    local dex = battler and catalogDex(dexOf(battler.species)) or nil
    -- Stadium 2 has no official Mega meshes. An active KASC form must reach
    -- Gold's pokemon.sprite front/animation hook, not keep the base-species
    -- mesh covering its billboard. Cleanup naturally restores that mesh.
    if battler and battler._ascMegaForm then dex = nil end
    local changed = session.at[side] ~= battler

    if changed then
      session.at[side] = battler
      if mon then mon.grow, mon.grewOwn = nil, nil end
      if mon and mon.rig and mon.state ~= "idle" then mon:play("idle") end
      session.gen2Entrance[side] = battler
      session.gen2Faint[side] = nil
    end

    local broken = session.broken and session.broken[side] or nil
    if broken and (broken.battler ~= battler or broken.dex ~= dex) then
      session.broken[side] = nil
      broken = nil
    end
    ...
    local faintSlide = screen.faintSlide
    local fainting = type(faintSlide) == "table" and faintSlide.side == side
    if fainting and session.gen2Faint[side] ~= battler and mon.rig then
      if type(mon.request) == "function" then pcall(mon.request, mon, "faint") end
      if mon.state == "faint" then session.gen2Faint[side] = battler end
    end

    mon.visible = (mon.rig ~= nil)
      and gen2SideVisible(screen, side, battler, mon)
    mon.model_matrix = nil
    if mon.rig then
      if session.gen2Entrance[side] == battler and mon.visible then
        if type(mon.beginGrow) == "function" then pcall(mon.beginGrow, mon) end
        if type(mon.request) == "function" then
          pcall(mon.request, mon, "entrance")
        end
        session.gen2Entrance[side] = nil
      end
      if mon.grow and type(mon.growScale) == "function" then
...
function Stadium.update(dt, battle, groundY)
  ...
  for _, side in ipairs({ "enemy", "player" }) do
    ...
    if battler and not showingTrainer(battle, side) then
      dex = catalogDex(session.transform[side]
        or dexOf(battler.mon and battler.mon.species))
    end
    ...
    if not broken then
      -- Gen1 has game.overworld, never Gold's game.world. Its explicit
      -- Stadium-2 adapter must allow the same validated static bind pose as
...
function Stadium.guard(side, mon, what, fn)
  ...
  -- again later.
  local brokenBattler = session and session.at and session.at[side] or nil
  local brokenDex = mon and mon.species or nil
  ...
  if session then
    session.broken[side] = {
      battler=brokenBattler,
      dex=brokenDex,
      what=what,
    }
    session.lastError = ("%s %s failed: %s")
      :format(tostring(side), tostring(what), tostring(err))
  end
...
function Stadium.draw(pull)
  ...
  for _, side in ipairs({ "enemy", "player" }) do
    ...
    if mon.rig and mon.visible and mon.model_matrix then
      local ok, err = Stadium.guard(side, mon, "draw", function()
        mon.rig:draw(mon.model_matrix, pull)
      end)
      if not ok then
        failures[#failures + 1] = tostring(side) .. ": " .. tostring(err)
      end
    end
  end
  if #failures > 0 then
    return false, "Stadium model draw failed (" .. table.concat(failures, "; ") .. ")"
  end
  return true
end
...
function Stadium.cast(shadowMap)
  ...
  for _, side in ipairs({ "enemy", "player" }) do
    local mon = session[side]
    if mon.rig and mon.visible and mon.model_matrix then
      local ok, err = Stadium.guard(side, mon, "cast", function()
  ...
  if #failures > 0 then
    return false, "Stadium model shadow failed (" .. table.concat(failures, "; ") .. ")"
  end
  return true
end
...
-- Exact screen-space ink receipt for the posed Stadium mesh currently drawn
-- on one battle side. BattleScene's generic camera prism is deliberately much
-- taller and wider than a Pokemon; using its top as a HUD head anchor sends
-- normal Stadium encounters hundreds of pixels above the visible model. Scan
-- the already-skinned pose bounds, transform their eight corners through the
-- same model/view-projection matrices used by the draw, and publish only that
-- visible model envelope. This is read-only presentation data: it does not
-- alter the rig, battle state, camera or animation.
local function projectPosedBounds(vp, modelMatrix, bounds, pw, ph)
  pw, ph = tonumber(pw), tonumber(ph)
  if not (type(vp) == "table" and type(modelMatrix) == "table"
      and type(bounds) == "table" and pw and pw > 0 and ph and ph > 0) then
    return nil
  end
  for index = 1, 6 do
    if type(bounds[index]) ~= "number" or bounds[index] ~= bounds[index] then
      return nil
    end
  end
  if not (bounds[4] > bounds[1] and bounds[5] > bounds[2]
      and bounds[6] > bounds[3]) then return nil end

  local mvp = Mat4.mul(vp, modelMatrix)
  local left, top, right, bottom
  for _, x in ipairs({ bounds[1], bounds[4] }) do
    for _, y in ipairs({ bounds[2], bounds[5] }) do
      for _, z in ipairs({ bounds[3], bounds[6] }) do
...
Stadium._projectPosedBounds = projectPosedBounds

local function receiptAnchors(hull, exactHeadY)
  if not (type(hull) == "table" and tonumber(hull[1])
      and tonumber(hull[2]) and tonumber(hull[3]) and tonumber(hull[4])) then
    return nil
  end
  local centreX = hull[1] + hull[3] * .5
  return { x=centreX, y=tonumber(exactHeadY) or hull[2] },
    { x=centreX, y=hull[2] + hull[4] }
end
...
function Stadium.visualReceipt(side, vp, pw, ph, renderToken, presentationMatrix)
  ...
  -- corners is only conservative: after camera/model rotation its highest
  ...
  if type(mon.rig.projectedBounds) == "function" then
    ...
    if mvpOk then
      local exactOk, left, top, width, height,
        ...
        mon.rig.projectedBounds, mon.rig, mvp, pw, ph)
  ...
  -- Fail open for an unskinned first frame or an older rig: the former
  -- conservative receipt is still much safer than dropping the HUD owner.
...
-- Stable world-space target data for procedural move effects.  Recalculate on
-- request so switches, Transform, species scale and send-out growth are all
-- reflected without letting the effect renderer reach into Stadium's session.
function Stadium.effectProfile(side)
  return refreshEffectProfile(side)
end

function Stadium.effectAnchor(side, kind)
  local profile = refreshEffectProfile(side)
  return BattleEffectAnchors.point(profile, kind), profile
end
