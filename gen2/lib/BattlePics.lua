...
-- Public, cached geometry receipt for presentation layers which need to
-- follow the visible Pokemon rather than the padded cartridge image. The GPU
-- readback happens once per source image and is shared by every subsequent
-- HUD frame; callers receive x, y, width and height in source pixels.
function BattlePics.inkRect(img)
  if not img then return nil end
  local hit = inkCache[img]
  if hit ~= nil then
  ...
  local ok = pcall(function()
    ...
    if x0 then receipt = { x0, y0, x1 - x0 + 1, y1 - y0 + 1 } end
  end)
  inkCache[img] = ok and receipt or false
  if type(receipt) == "table" then
    return receipt[1], receipt[2], receipt[3], receipt[4]
  end
  return nil
end
