...
function StadiumMon:attackGen2(moveIndex, def)
  ...
  -- Only an extractor-supplied, non-provisional move table may choose a
  -- move-specific clip.  The earlier family heuristic selected an arbitrary
  -- imported performance when the table contained only placeholder slot 0;
  -- that looked animated but could not prove it belonged to the move.  Return
  -- false instead so BattleStadiumAnimations requests the named VASC
  -- `attack_default` state.  Exact Stadium-1/shared-range mappings still win.
  if moveIndex <= StadiumPack.N_MOVES and not provisionalMoveRouting(model) then
    return self:attack(moveIndex)
  end
  return false
end
