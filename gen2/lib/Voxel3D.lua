...
local worldPresentationReceipt = nil

-- One receipt owns every pixel written into a Gen-2 world canvas.  It is
-- sampled by VoxelScene, not here, so projection, backdrop and weather cannot
-- observe different device orientations within one frame.
function Voxel3D.setWorldPresentation(receipt)
  if type(receipt) ~= "table"
      or receipt.schema ~= CanvasPresentation.WORLD_PRESENTATION_SCHEMA then
    return false
  end
  worldPresentationReceipt = receipt
  if type(CanvasPresentation.setWorldPresentation) == "function" then
    CanvasPresentation.setWorldPresentation(receipt)
  end
  return true
end

function Voxel3D.worldPresentation()
  return worldPresentationReceipt
end

local function worldClipScale()
  if type(CanvasPresentation.worldClipScale) == "function" then
    return CanvasPresentation.worldClipScale(worldPresentationReceipt)
  end
  return 1, -1
end

local function worldClipMatrix()
  local sx, sy = worldClipScale()
  return Mat4.scale(sx, sy, 1)
end
...
local SHADER = [[
  ...
  // Repeated Johto tree hulls attach one compact translation per instance.
  // Ordinary meshes leave the attribute at the graphics default (0,0,0), so
...
local function newDepth(w, h)
  -- Mobile water already uses the flat safe path.  A readable full-scene
  -- depth texture there buys nothing, but doubles an expensive allocation and
  -- is a common GLES failure point during first world/battle construction.
  if MOBILE_RUNTIME then return nil end
...
-- Hardware instancing is an acceleration only. Drivers which cannot attach a
-- per-instance attribute keep the exact expanded geometry fallback.
...
function Voxel3D.viewProjection(cx, cy, vw, vh)
  ...
  if cam then
    ...
    -- them; only the platform-owned clip receipt is added, for the same
    -- canvas reason as every other branch here
    if cam.view and cam.proj then
      ...
      return Mat4.mul(Mat4.mul(worldClipMatrix(), cam.proj), cam.view)
    end
    ...
    -- the same clip-space receipt the orbit needs, for the same reason: we
    -- bypass LOVE's transform_projection. It is Y on desktop/Android and on
    -- ordinary iOS orientation, X only for iOS's flipped presenter.
    proj = Mat4.mul(worldClipMatrix(), proj)
  ...
  local proj = Mat4.perspective(fov, vw / vh,
                                math.max(1, dist * 0.05), dist * 4 + 4096)
  -- Consume the platform's clip receipt. Mat4.perspective emits textbook GL
  -- clip space with +Y up, but we bypass LOVE's own transform_projection.
  -- The established correction is Y everywhere except flipped iOS, whose
  -- presenter moves the reflection to X. Winding can change freely because
  -- this pass draws with culling off.
  proj = Mat4.mul(worldClipMatrix(), proj)
...
function Voxel3D.backdrop(image, tint)
  ...
  local dx, dy, dr, dsx, dsy = CanvasPresentation.imageDraw(
    canvasW, canvasH, iw, ih, nil, worldPresentationReceipt)
...
function Voxel3D.backdropWindows(scene, regions, sourceW, sourceH)
  ...
  local ok = pcall(function()
    ...
    local function regionShape(region)
      if region.shape == "rect" then
        local rw = region.w * sx
        local rh = region.h * sy
        gfx.rectangle("fill", CanvasPresentation.rectX(
                        ox + region.x * sx, rw, canvasW,
                        nil, worldPresentationReceipt),
                      CanvasPresentation.rectY(oy + region.y * sy,
                                               rh, canvasH,
                                               nil, worldPresentationReceipt),
                      rw, rh)
      elseif region.shape == "ellipse" then
        gfx.ellipse("fill", CanvasPresentation.pointX(
                      ox + (region.x + region.w * .5) * sx, canvasW,
                      nil, worldPresentationReceipt),
        ...
        local points = {}
        for i=1,#region.points,2 do
          points[#points + 1] = CanvasPresentation.pointX(
            ox + region.points[i] * sx, canvasW,
            nil, worldPresentationReceipt)
      ...
      for ri, region in ipairs(regions) do
        ...
        for i = 1, 13 do
          ...
          if pointIn(region, x, y) then
            ...
            gfx.circle("fill", CanvasPresentation.pointX(
                         ox + x * sx, canvasW,
                         nil, worldPresentationReceipt),
                       CanvasPresentation.pointY(
                         oy + y * sy, canvasH,
...
-- Camera-facing half-plane used by VASC's interior cutaway. Passing no plane
-- restores the ordinary opaque world contract.
function Voxel3D.setCutaway(nx, nz, offset)
  if not (active and activeShader) then return false end
  local value
  if nx ~= nil and nz ~= nil and offset ~= nil then
    value = { tonumber(nx) or 0, tonumber(nz) or 0, tonumber(offset) or 0, 1 }
  else
    value = { 0, 0, 0, 0 }
  end
  return pcall(activeShader.send, activeShader, "cutaway", value)
end
...
function Voxel3D.draw(mesh, texture, model, pull, sunModel)
  ...
  local function drawPlain(value)
    if not value then return end
    if value.__voxelMeshParts then
      if texture then value:setTexture(texture) end
      for _, part in ipairs(value.parts or {}) do love.graphics.draw(part) end
    else
      if texture then value:setTexture(texture) end
  ...
  if mesh.__voxelMeshBundle then
    ...
    for _, group in ipairs(mesh.instances or {}) do
      for _, part in ipairs(group.meshes or {}) do
        if texture then part:setTexture(texture) end
        love.graphics.drawInstanced(part, group.count)
      end
    end
  else
    drawPlain(mesh)
...
function Voxel3D.project(wx, wy, wz)
  ...
  -- viewProjection already consumed this frame's platform clip receipt, so
  -- both axes map the same way here -- no second flip
...
function Voxel3D.captureSceneState()
  return {
    ...
    worldPresentationReceipt = worldPresentationReceipt,
  }
end
...
function Voxel3D.restoreSceneState(state)
  ...
  worldPresentationReceipt = state.worldPresentationReceipt
  if type(CanvasPresentation.setWorldPresentation) == "function" then
    CanvasPresentation.setWorldPresentation(worldPresentationReceipt)
  end
  return true
end
