...
-- The standalone Gold bridge supplies the live Game2 instance as V.game.
-- Fall back to the original Gen-1 singleton when this module is used by the
-- upstream Dramatic Shapes pipeline. Keeping this lookup here lets the same
-- camera rig drive both generations without teaching VoxelScene about hosts.
local function hostGame()
  if V and type(V.game) == "table" then return V.game end

  local ok2, Game2 = pcall(require, "src.core.Game2")
  if ok2 and type(Game2) == "table" and (Game2.world or Game2.stack) then
    return Game2
  end
...
FirstPerson.hostGame = hostGame
FirstPerson.hostWorld = hostWorld
...
function FirstPerson.onTop()
  ...
  -- Gold/Game2 owns a live `world` and uses an EMPTY stack for normal free
  -- roam; menus/dialogs are pushed above it. Check that host first in case a
  -- compatibility alias named `overworld` also exists. Gen 1 keeps the actual
...
function FirstPerson.update(dt)
  ...
  if engagedNow and not wasEngaged then
    local world = hostWorld()
    local facing = world and world.player and world.player.facing or nil
    FirstPerson.yaw = FACING_ANGLE[facing] or 0
    FirstPerson.pitch = FirstPerson.PITCH_DEFAULT
  end
...
function FirstPerson.install(game)
  game = game or hostGame()
  if installed then
    if not game or game == installedGame then return true end
    return false, "first-person input hooks already installed on another game host"
  end
  if type(game) ~= "table" then
    return false, "no live game host for first-person input hooks"
  end
  ...
  do
    ...
    love.mousereleased = function(x, y, button, istouch, presses)
      -- If this camera never claimed the button, preserve the normal engine
      -- mouse release path exactly.
      if not mouseHeld[button] then
        if inner then return inner(x, y, button, istouch, presses) end
        return
      end
      local Input = require("src.core.Input")
  ...
  do
    ...
    function Game:focus(f)
      ...
      stick.x, stick.y = 0, 0
      mouseDX, mouseDY = 0, 0
      if inner then return inner(self, f) end
    end
  end
  ...
  do
    ...
    function Game:joystickremoved(joystick)
      ...
      if inner then return inner(self, joystick) end
    end
  end
  return true
end
