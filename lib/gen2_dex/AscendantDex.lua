...
local function spriteMode()
  ...
  if value ~= "auto" and value ~= "game" and value ~= "cobblemon" then return "kasc_crystal" end
  return value
end
...
local function wrappedDescription(game, def, owned, lang)
  ...
  local raw = owned and (entry.inlineText or entry.text and game.data.text[entry.text])
...
for _,class in ipairs({ListScreen, EntryScreen, AreaScreen})do
