...
function OverworldBattle.update(dt)
  ...
  if shot and shot.canvas then
    ...
    -- Keep both HUDs in the engine's own battle frame.  An earlier RC moved
    -- them to the physical window edges, but at large integer framebuffer
    -- scales that made each block enormous, clipped the enemy name at x=0,
    -- and separated the HUDs from the command box they belong to.  Leaving
    -- them in-frame also means BATTLE SIZE and the renderer's UI scale apply
    -- to every piece of battle furniture consistently.
    session.snapped = false
  end
  session.shot = shot
end
