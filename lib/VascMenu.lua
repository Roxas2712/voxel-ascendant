...
local SETTING_HELP_DE = {
  ...
  modernDexSpriteSource = "Bildquelle ausschließlich im ModernDex: KASC "
    ...
    .. "Cartridge-Bild. COBBLEMON 3D zeigt enthaltene animierte Modelle mit Sprite-Ersatz bei fehlenden Arten. HD ANIMATED nutzt installierte Animationen mit "
...
function VascMenu.install(mod, opts)
  ...
  if inbox then
    local errors=V.require("ErrorsMenu").install(mod,inbox,{language=function()return languageCode(mod)end,supportProvider=config.diagnostics,openDiagnostics=function(game)return game.stack:push(newDiagnostics(mod,game))end,session=config.diagnostics.sessionId,notify=function(code)V.require("ShortcutToast").notify("ASCENDANT ERRORS",code.." - open Errors")end})
