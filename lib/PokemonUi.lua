...
PokemonUi.REQUIREMENTS = {
  pc_box = {
    ...
    actions = {
      ...
      "change_box", "print_box", "search", "search_locate", "cancel",
    },
    events = {
...
local MODEL_KEYS = arraySet({
  ...
  "focus", "selection", "zones", "availability", "surfaceData", "search",
})
...
local SEARCH_KEYS = arraySet({ "active", "mode", "query", "results" })
local SEARCH_RESULT_KEYS = arraySet({
  "id", "box", "slot", "sourceBox", "sourceSlot", "pokemon",
})
...
local function validSearch(value)
  if type(value) ~= "table" or not onlyKeys(value, SEARCH_KEYS)
      or type(value.active) ~= "boolean"
      or not nonempty(value.mode) or type(value.query) ~= "string"
      or not isExactArray(value.results) then
    return false
  end
  if value.mode ~= "name" and value.mode ~= "type"
      and value.mode ~= "gender" then return false end
  for _, result in ipairs(value.results) do
    if type(result) ~= "table" or not onlyKeys(result, SEARCH_RESULT_KEYS)
        or not nonempty(result.id) or not integer(result.box, 1)
        or not integer(result.slot, 1)
        or not optionalInteger(result.sourceBox, 1)
        or not optionalInteger(result.sourceSlot, 1)
        or (result.sourceBox ~= nil and result.sourceBox ~= result.box)
        or (result.sourceSlot ~= nil and result.sourceSlot ~= result.slot)
        or not validatePokemonDescriptor(result.pokemon) then
      return false
    end
  end
  return true
...
local function validateModel(raw, host, surface, session, hostDef)
  ...
  if surface == "pc_box" and not validSearch(model.search) then
    return nil, "model-search-invalid"
  elseif model.search ~= nil and not validSearch(model.search) then
    return nil, "model-search-invalid"
  end
  local seen = {}
...
local ACTION_PAYLOAD_KEYS = {
  ...
  search=arraySet({ "modelRevision", "mode", "query" }),
  search_locate=arraySet({ "modelRevision", "target" }),
...
local function validateActionPayload(action, raw, currentRevision)
  ...
  elseif action == "search" then
    if payload.mode ~= "name" and payload.mode ~= "type"
        and payload.mode ~= "gender" then
      return nil, "action-search-mode-invalid"
    end
    if type(payload.query) ~= "string" then
      return nil, "action-search-query-invalid"
    end
  elseif action == "search_locate" then
    if not validateLocation(payload.target, true) then
      return nil, "action-search-target-invalid"
    end
...
local function wrapController(controller, receipt, life, provider, host,
  ...
  local rawInputKinds = arraySet({
    "textinput", "textedited", "keypressed", "pointerpressed", "endSearch",
  })
  ...
  -- Raw IME/pointer values cross one bounded, fail-open wrapper. The host
  -- never receives provider authority and the provider never receives Game.
  function wrapped:inputSeam(kind, ...)
    if not life.active or not rawInputKinds[kind] then return false end
    if kind ~= "pointerpressed" and kind ~= "endSearch" then
      local active = controller.isTextInputActive
      if type(active) ~= "function" then return false end
      local okActive, isActive = pcall(active, controller)
      if not okActive or isActive ~= true then return false end
    end
    local method = controller[kind]
  ...
  function wrapped:isTextInputActive()
    if not life.active or type(controller.isTextInputActive) ~= "function" then
      return false
    end
    local ok, active = pcall(controller.isTextInputActive, controller)
