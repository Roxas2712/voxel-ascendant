...
local backdropCompositions = {}
local backdropFailures = {}
...
-- Pick two conservative foot marks from an arbitrary 1280x800 ARENA image.
-- This is intentionally image-only: future injector packs may replace or
-- shuffle scenery without knowing anything about the ROM map. Opaque, locally
-- calm lower-middle patches rank above noisy foliage/architecture; transparent
-- sky, frame edges and cramped pairs are rejected. Reviewed metadata remains
-- the fallback when no credible pair can be found.
local function analyseBackdropComposition(data, width, height, analyseSky)
  if not (data and type(data.getPixel) == "function"
      and width and height and width > 0 and height > 0) then return nil end

  local function pixel(x, y)
    x = math.max(0, math.min(width - 1, math.floor(x + .5)))
    y = math.max(0, math.min(height - 1, math.floor(y + .5)))
    -- The complete backdrop decode/upload path already runs inside one pcall;
    -- avoid thousands of nested protected calls during this bounded scan.
    local r, g, b, a = data:getPixel(x, y)
    local light = .2126 * (r or 0) + .7152 * (g or 0) + .0722 * (b or 0)
    return light, a or 0
  end
  ...
  local function candidate(nx, ny, targetX, targetY)
    ...
    -- The band straddles the feet and extends below them. A stand point on
    -- sky/transparent paint fails, while grass, floorboards and paths retain
    -- a continuous support patch. Sparse sampling keeps one load inexpensive.
    for oy = -12, 28, 8 do
      for ox = -56, 56, 16 do
        local l, a = pixel(cx + ox, cy + oy)
        local lr, ar = pixel(cx + ox + 5, cy + oy)
        local ld, ad = pixel(cx + ox, cy + oy + 5)
        count = count + 1
        if a >= .72 and ar >= .72 and ad >= .72 then opaque = opaque + 1 end
        rough = rough + math.abs(l - lr) + math.abs(l - ld)
      end
    end
    local coverage = opaque / math.max(1, count)
    if coverage < .86 then return nil end
    rough = rough / math.max(1, count)
    -- Vertical position is the stronger prior.  Detailed paintings often
    -- contain an opaque crate, rail or dais above the actual floor; treating
    -- that small object as equivalent support is what left short Pokemon
    -- visibly hovering in the Lighthouse.  The support test may still move
    -- away from the prior when the authored floor is absent there.
    local targetPenalty = math.abs(nx - targetX) * 1.7
                        + math.abs(ny - targetY) * 10.0
    return coverage * 4 - rough * 3.2 - targetPenalty
  end

  local function ranked(x0, x1, targetX, targetY)
    local out = {}
    for nx = x0, x1 + .0001, .035 do
      for ny = .52, .76 + .0001, .03 do
  ...
  local players = ranked(.20, .47, .34, .68)
  local enemies = ranked(.53, .80, .66, .64)
  ...
  -- Voxel-sky art carries real PNG alpha.  Derive only a conservative
  -- all-transparent rectangle from that channel; engine OUTDOOR status never
  -- participates.  A coarse maximal-rectangle scan samples centre and four
  -- near-corners per cell, then shrinks the winner by one cell on every edge
  -- so clouds/sun/events cannot be aimed at an opaque frame pixel.
  if analyseSky == true then
    local columns, rows = 64, 40
    local maxRow = math.floor(rows * .72)
    local transparent = {}
    local function clearCell(ix, iy)
      local x0, x1 = ix / columns * width,
        (ix + 1) / columns * width - 1
      local y0, y1 = iy / rows * height,
        (iy + 1) / rows * height - 1
      for _, point in ipairs({
        { .5, .5 }, { .12, .12 }, { .88, .12 },
        { .12, .88 }, { .88, .88 },
      }) do
        local _, alpha = pixel(x0 + (x1 - x0) * point[1],
                               y0 + (y1 - y0) * point[2])
        if alpha > .12 then return false end
      end
      return true
    end
    for y = 0, maxRow - 1 do
      ...
      for x = 0, columns - 1 do
        transparent[y + 1][x + 1] = clearCell(x, y)
      end
    end
    local heights = {}
    local winner
    for row = 1, maxRow do
      for column = 1, columns do
        heights[column] = transparent[row][column]
          and ((heights[column] or 0) + 1) or 0
      end
      local stack = {}
      for column = 1, columns + 1 do
        local height = column <= columns and (heights[column] or 0) or 0
        local start = column
        while #stack > 0 and stack[#stack].height > height do
          local item = table.remove(stack)
          local widthCells = column - item.start
          local area = item.height * widthCells
          local top = row - item.height
          if area > 0 and (not winner or area > winner.area
              or area == winner.area and top < winner.top) then
            winner = { area=area, left=item.start - 1, top=top,
                       width=widthCells, height=item.height }
          end
          start = item.start
        end
        if #stack == 0 or stack[#stack].height < height then
          stack[#stack + 1] = { start=start, height=height }
        end
      end
    end
    if winner then
      local left = winner.left + 1
      local top = winner.top + 1
      local apertureWidth = winner.width - 2
      local apertureHeight = winner.height - 2
      local aperture = {
        x=left / columns, y=top / rows,
        width=apertureWidth / columns,
        height=apertureHeight / rows,
      }
      if apertureWidth >= 6 and apertureHeight >= 3
          and aperture.width * aperture.height >= .012
          and aperture.y <= .52 then
        best = best or { source="alpha-sky-only/v1" }
        best.skyAperture = aperture
      end
    end
  end
  return best
...
function VoxelBattleStage.retryFailedImages()
  for _, cache in ipairs({ diskTextures, backdropTextures }) do
...
local function validNormalizedRect(rect)
  if type(rect) ~= "table" then return false end
  local x, y, width, height = rect.x, rect.y, rect.width, rect.height
  return finite(x) and finite(y) and finite(width) and finite(height)
    and x >= 0 and y >= 0 and width > 0 and height > 0
    and x + width <= 1.0000001 and y + height <= 1.0000001
end
...
local function authoredSpec(arena)
  ...
  -- Gen 2's private BattleBackdrop resolver already selected the authoritative
  -- catalog entry.  Normalize that compact immutable record into the richer
  -- Gen-1 stage contract without mutating the source table.  Catalog entries
  -- intentionally omit legacy camera/anchor boilerplate: bitmap composition
  -- supplies safe feet, while zero world offsets remain the deterministic
  -- fallback until the image analysis has completed.
  local direct = arena and arena.backdropSpec
  if direct ~= nil then
    if type(direct) ~= "table" or type(direct.path) ~= "string"
        or direct.path == ""
        or (direct.width ~= nil
            and direct.width ~= VoxelBattleStage.BACKDROP_W)
        or (direct.height ~= nil
            and direct.height ~= VoxelBattleStage.BACKDROP_H)
        or (direct.outdoor ~= nil and type(direct.outdoor) ~= "boolean")
        or (direct.voxelSky ~= nil and type(direct.voxelSky) ~= "boolean")
        or (direct.actorScale ~= nil
            and (not finite(direct.actorScale) or direct.actorScale < 0.5
                 or direct.actorScale > 2.25)) then
      return nil
    end
    if direct.skyAperture ~= nil
        and (direct.voxelSky ~= true
             or not validNormalizedRect(direct.skyAperture)) then
      return nil
    end
    local function anchor(side, family)
      local value = type(family) == "table" and family[side] or nil
      if value == nil then return { x=0, y=0, z=0 } end
      if not (type(value) == "table" and finite(value.x)
          and finite(value.y) and finite(value.z)) then return nil end
      return { x=value.x, y=value.y, z=value.z }
    end
    local player = anchor("player", direct.anchors)
    local enemy = anchor("enemy", direct.anchors)
    if not (player and enemy) then return nil end
    local trainerAnchors
    if direct.trainerAnchors ~= nil then
      local trainerPlayer = anchor("player", direct.trainerAnchors)
      local trainerEnemy = anchor("enemy", direct.trainerAnchors)
      if not (trainerPlayer and trainerEnemy) then return nil end
      trainerAnchors = { player=trainerPlayer, enemy=trainerEnemy }
    end
    return {
      camera = "3X",
      path = direct.path,
      width = VoxelBattleStage.BACKDROP_W,
      height = VoxelBattleStage.BACKDROP_H,
      outdoor = direct.outdoor == true,
      voxelSky = direct.voxelSky == true,
      skyAperture = direct.skyAperture,
      actorScale = tonumber(direct.actorScale) or 1,
      clockTint = false,
      anchors = { player=player, enemy=enemy },
      trainerAnchors = trainerAnchors,
      catalogKey = arena.backdropKey,
    }
  end
  ...
  -- Dynamic sky behind a full-frame bitmap is explicitly authored.  Old and
  -- third-party scenery records therefore remain closed even when their PNG
  -- happens to contain alpha.  The optional aperture is a normalized receipt
  -- for the battle camera/SkyEvents bridge and is valid only with the opt-in.
  if spec.voxelSky ~= nil and type(spec.voxelSky) ~= "boolean" then return nil end
  if spec.skyAperture ~= nil then
    if spec.voxelSky ~= true or not validNormalizedRect(spec.skyAperture) then
      return nil
    end
  end
...
local function authoredPath(arena, spec)
  ...
  local base = spec and spec.path or nil
  if frlg then
    local mode = style and style.arenaArtMode or "mix"
    if mode == "frlg"
        or mode == "mix" and style and style.arenaArtPick == "frlg" then
      base = frlg
    end
  end

  -- Content Selector backdrops are generation-scoped immutable assets. The
  -- package catalogue still owns camera, weather, sky and actor metadata; the
  -- verified preset may replace only the selected PNG. Prefer the real map id
  -- and retain style/catalog aliases for packs authored before map ids were
  -- exposed by the injector.
  local okContent, content = pcall(V.require, "LocalContent")
  if okContent and type(content) == "table"
      and type(content.resolveBackdrop) == "function" then
    local seen = {}
    for _, slot in ipairs({
      arena and arena.backdropMapId,
      style and style.mapId,
      arena and arena.backdropKey,
      style and style.id,
      arena and arena.map and (arena.map.id
        or arena.map.def and arena.map.def.id),
    }) do
      slot = type(slot) == "string" and slot:upper() or nil
      if slot and slot ~= "" and not seen[slot] then
        seen[slot] = true
        local okResolve, imported = pcall(content.resolveBackdrop, slot, nil)
        if okResolve and type(imported) == "string" and imported ~= "" then
          return imported
        end
      end
    end
...
local function assetSourcePath(path)
  if type(path) ~= "string" or path == "" then return nil end
  -- UserFiles.path() returns an already verified absolute host path. Bundled
  -- catalogue paths remain package-relative and are resolved through V.path.
  -- Current Gen1Recomp exposes installed mod files as PhysFS-rooted `mods/`
  -- (and derived files as `save/`) paths rather than host-absolute paths; they
  -- are equally complete and must not receive the mod prefix a second time.
  if path:sub(1, 1) == "/" or path:match("^%a:[/\\]") then return path end
  local normalized = path:gsub("\\", "/")
  if normalized:match("^mods/") or normalized:match("^save/") then return path end
  return type(V.path) == "string" and (V.path .. "/" .. path) or nil
end
...
local function backdropImage(arena, outdoor)
  ...
  -- An explicitly authored voxel-sky opening is valid even when the engine
  -- labels the containing map INDOOR (lighthouse, ship, Burned Tower, Gyms).
  -- Closed paintings retain the historical environment match and therefore
  -- fail safely rather than loading an outdoor fallback in a room.
  if not (spec and (spec.voxelSky or spec.outdoor == outdoor)
      and type(V.path) == "string") then
    return nil
  end
  local selectedPath = authoredPath(arena, spec)
  local key = tostring((style and style.id) or arena.backdropKey or "arena")
    .. ":" .. selectedPath
  ...
  local ok, image = pcall(function()
    local path = assetSourcePath(selectedPath)
    if not path then error("Arena Scenery path unavailable") end
    ...
    composition = analyseBackdropComposition(
      data, w, h, spec.voxelSky == true)
    if arena and arena.backdropSpec
        and not (type(composition) == "table"
                 and type(composition.player) == "table"
                 and type(composition.enemy) == "table") then
      if data.release then pcall(data.release, data) end
      error("Arena Scenery has no safe actor-ground pair")
    end
    local okImage, out = pcall(love.graphics.newImage, data)
    if data.release then pcall(data.release, data) end
    if not okImage or not out then
  ...
  if not (ok and image) then
    backdropFailures[key] = tostring(image or "Arena Scenery load failed")
    cacheImageFailure(backdropTextures, key, retryable, retrying)
    return nil
  end
  backdropFailures[key] = nil
  backdropCompositions[key] = composition or false
...
-- Suggested screen-space feet for the loaded painting. BattleScene applies
-- them only when the projected pair is visibly unsafe, keeping established
-- reviewed compositions stable while allowing replaced/shuffled art to
-- recover without map-specific Lua code.
function VoxelBattleStage.presentationComposition(arena, trainer)
  local spec = authoredSpec(arena)
  if not spec then return nil end
  local path = authoredPath(arena, spec)
  local style = arena and arena.arenaStyle
  local key = style and path and (style.id .. ":" .. path) or nil
  local value = key and backdropCompositions[key]
  if type(value) ~= "table" then return nil end
  if trainer then
    return {
      player=value.trainerPlayer or value.player,
      enemy=value.trainerEnemy or value.enemy,
      source=value.source,
    }
  end
  return value
...
-- Read-only diagnostic receipt for a declined authored bitmap.  The normal
-- renderer still fails closed to the native battle view; QA can distinguish
-- a missing file, dimensions change, unsafe ground and a transient GPU upload
-- without weakening any acceptance threshold or exposing the caches.
function VoxelBattleStage.backdropFailure(arena)
  local spec = authoredSpec(arena)
  local path = spec and authoredPath(arena, spec)
  if not path then return nil end
  local style = arena and arena.arenaStyle
  local key = tostring((style and style.id) or arena.backdropKey or "arena")
    .. ":" .. path
  return backdropFailures[key]
end

-- Public read-only policy.  A validated Injector BattleLayout profile may
-- override the bundled scenery receipt for the matching map/stage.  No flag
-- means no dynamic voxel sky; callers must use a static clear behind alpha.
function VoxelBattleStage.skyPolicy(arena)
  local spec = authoredSpec(arena)
  if not spec then
    return { voxelSky=false, skyAperture=nil, source="no-authored-backdrop" }
  end
  local style = arena and arena.arenaStyle
  local path = authoredPath(arena, spec)
  local compositionKey = style and path
    and (tostring(style.id) .. ":" .. path) or nil
  local composition = compositionKey and backdropCompositions[compositionKey]
  local derivedAperture = type(composition) == "table"
    and composition.skyAperture or nil
  local aperture = validNormalizedRect(spec.skyAperture)
    and spec.skyAperture
    or (spec.voxelSky == true and validNormalizedRect(derivedAperture)
        and derivedAperture or nil)
  local policy = {
    voxelSky = spec.voxelSky == true,
    skyAperture = validNormalizedRect(aperture)
      and {
        x=aperture.x, y=aperture.y,
        width=aperture.width, height=aperture.height,
      } or nil,
    source = spec.skyAperture and "reviewed-arena-scenery"
      or aperture and "alpha-sky-aperture/v1"
      or "reviewed-arena-scenery",
  }
  local okLayout, layout = pcall(V.require, "BattleLayout")
  if okLayout and type(layout) == "table"
      and type(layout.stageContext) == "function"
      and type(layout.skyPolicy) == "function" then
    local okContext, context = pcall(layout.stageContext, arena,
                                     arena and arena.map)
    local okPolicy, imported = false, nil
    if okContext then okPolicy, imported = pcall(layout.skyPolicy, context) end
    if okPolicy and type(imported) == "table" then
      local aperture = imported.skyAperture
      policy = {
        voxelSky = imported.voxelSky == true,
        skyAperture = validNormalizedRect(aperture) and {
          x=aperture.x, y=aperture.y,
          width=aperture.width, height=aperture.height,
        } or nil,
...
function VoxelBattleStage.presentationTint(arena, skyExposed, weather)
  local spec = authoredSpec(arena)
  if spec and (spec.outdoor or spec.voxelSky) and skyExposed == true then
    local tint = Voxel3D.tint or { 1, 1, 1 }
    if spec.outdoor == true and weather == "snow" then
      -- Bright/cool multiplication affects only opaque backdrop pixels; real
      -- PNG alpha openings remain untouched for the live skybox.
      return { (tint[1] or 1) * 1.16,
               (tint[2] or 1) * 1.20,
               (tint[3] or 1) * 1.28 }
    elseif spec.outdoor == true
        and (weather == "rain" or weather == "storm") then
      return { (tint[1] or 1) * .72,
               (tint[2] or 1) * .78,
               (tint[3] or 1) * .88 }
    elseif spec.outdoor == true and weather == "fog" then
      return { (tint[1] or 1) * .90,
               (tint[2] or 1) * .94,
               (tint[3] or 1) * .94 }
    end
    return tint
...
function VoxelBattleStage.drawBackdrop(arena, outdoor, image, weather)
  -- Authored ARENA stages own the whole picture.  Gen-2 MAP stages may attach
  -- the same validated location bitmap as a panorama-only background while
  -- retaining their real voxel terrain and physical combatant anchors.
  if not (arena and (arena.arenaStyle or arena.panoramaBackdrop == true)) then
    return false
  end
  image = image or backdropImage(arena, outdoor and true or false)
  if not image then return false end
  local drawn = Voxel3D.backdrop(
    image, VoxelBattleStage.presentationTint(arena, outdoor, weather))
