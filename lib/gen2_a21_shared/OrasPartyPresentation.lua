...
-- Optional caller-local chrome.  The default is deliberately nil: Gen-1 and
-- every existing host keep the authored ORAS orange unless they opt in.
...
local function drawTypePill(kind, x, y, w)
  local accent = typeAccentFor(kind)
  local canonical = canonicalType(kind)
  local label = TypeChart.displayName(canonical) or tostring(canonical or "---")
  rounded(accent, x, y, w, 13, 3)
  outline(C.navy, x, y, w, 13, 3, 1, 0.65)
  centeredText(fitText(label, w - 6), x + 2, y + 3, w - 4,
...
local function drawTypeIcon(kind, x, y, size)
  ...
  outline(C.navy, x, y, size, size, math.max(3, size * .22), 1, .75)
  drawTypeGlyph(kind, x + size / 2, y + size / 2, size * .58)
end

local function drawTypeBadge(game, mon, x, y, size)
  local kinds = monTypes(game, mon)
  if kinds[2] then
    local small = math.floor((size - 2) / 2)
    drawTypeIcon(kinds[1], x, y + math.floor((size - small) / 2), small)
    drawTypeIcon(kinds[2], x + small + 2,
      y + math.floor((size - small) / 2), small)
    return
  end
...
local function resolveMonPalette(data, mon)
  -- Gen-II fronts carry their two middle colors in gen2Palettes rather than
  -- Gen-I's PaletteFX pack.  Using the Gen-I resolver here made perfectly
  -- valid Crystal art appear gray in Team/Detail/Nickname previews.
  if data and data.gen2Palettes and mon and mon.species then
    local ok, Palettes = pcall(require, "src.world.gen2.Palettes")
    if ok and Palettes and type(Palettes.monColors) == "function" then
      local colors = Palettes.monColors(data.gen2Palettes, mon.species,
...
local function drawTeamView(self)
  ...
  for index = 1, Model.PARTY_CAPACITY do
    ...
    if mon then
      ...
      if not egg and isActive(self, mon) then
...
local function drawDetail(self)
  ...
  if egg then
    ...
    drawTypeBadge(self.game, mon, x + 13, y + 48, 24)
  end
  drawMon(self.game, mon, x + 45, y + 28, 90, 56,
...
local function partySubmenuState(self)
  ...
  -- Gold/Silver/Crystal keeps this menu inside a native object.  Accept that
  -- shape without taking input or swap ownership away from PartyMenu.lua.
...
local function hostStorageScreen(model, state)
  ...
  local box = hostZoneSlots(model.zones and model.zones.box)
  ...
  if type(state.carry) == "table" then
    ...
    local source = kind == "party" and party[index]
      or tonumber(state.carry.box) == currentBox and box[index] or nil
    held = {
      kind=kind,
      index=index,
...
local function drawPresentation(self)
  drawBackground()
  drawHeader(self)
...
function P.draw(self)
  local primary, bright = normalizeChromeAccent(
    type(self) == "table" and self.__vascOrasPartyChromeAccent or nil)
  if not primary then return drawPresentation(self) end

  -- LÖVE renders on one Lua thread, so a tightly scoped palette swap keeps
  -- the established renderer intact while guaranteeing restoration on error.
  local nativeOrange, nativeOrange2 = C.orange, C.orange2
  C.orange, C.orange2 = primary, bright
  local ok, err = pcall(drawPresentation, self)
...
function P.decoratePartyMenu(menu, opts)
  ...
  if menu.__vascOrasPartyDecorated then
    ...
    menu.battle = battle
    if opts.chromeAccent ~= nil then
      menu.__vascOrasPartyChromeAccent = opts.chromeAccent
    end
  ...
  if type(nativeUpdate) == "function" then
    menu.update = function(self, ...)
      ...
      -- Reflect Gen-II's native two-step SWITCH state in the visual marker;
      -- PartyMenu remains the only authority that mutates the party.
...
P.normalizeChromeAccent = normalizeChromeAccent
