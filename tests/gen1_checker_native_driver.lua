return function(game)
 ...
 for _,id in ipairs({'FUCHSIA_CITY','ROUTE_12','ROUTE_10','ROUTE_20'})do
  U.teleport(game,id,4,4,'up');U.wait(2)
  local map=game.overworld.map;local x,y=4,4
  for i,block in ipairs(map.def.blocks)do
   if block==122 then x=((i-1)%map.def.width)*2;y=math.floor((i-1)/map.def.width)*2;break end
  end
  assert(map:isWalkableCell(x,y),'chosen paving cell not walkable')
 ...
 print('PASS_CHECKER_PAVING');love.event.quit()
end
