return function(game)
 ...
 for _,id in ipairs({'GOLDENROD_CITY','ECRUTEAK_CITY','OLIVINE_CITY','NEW_BARK_TOWN'})do
  ...
  print('PASS_NATIVE_ROOF_PIXELS',id,w*h);local scratch=love.image.newImageData(w,h);scratch:paste(map.renderer._stadiumAtlasData,0,0,0,0,w,h);local styled=V.require('GoldColorAtlas').styleWalls(geometry,scratch,map,'gbc');scratch:release();print('STYLED_WALL_TILES',id,styled);assert(styled>0,'no native wall match '..id)
  do
   chosen.cameraMode='third';local F=V.require('FirstPerson');F.yaw=math.pi;F.pitch=.22;wait(20)
   love.graphics.captureScreenshot(function(data)local f=assert(io.open(views..'/'..id..'.png','wb'));f:write(data:encode('png'):getString());f:close();data:release()end);wait(4)
  end
