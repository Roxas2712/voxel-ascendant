-- Native release regression. Requires disposable vasc-301-native identity,
-- imported Gen1/Gen2 data, HD content, Stadium 2 caches, and SHOT_DIR.
-- Disposable native game QA only. Exercises the actual VASC setting row.
return function(game)
  io.stdout:setvbuf('no')
  assert(os.getenv('POKEPORT_IDENTITY')=='vasc-301-native')
  local U={}
  function U.wait(n) for _=1,n do coroutine.yield() end end
  function U.shot(g,path) g.capturePath=path;U.wait(3);return true end
  ...
  verify(false) -- must work before movement or a map event
  U.hold(game,'left',12);U.wait(15);verify(false)
  assert(game.save.player.characterKey==identity,'character identity changed')
  assert(U.shot(game,dir..'/player-off.png'))
  row.descriptor.step(game,1)
  verify(true)
  U.hold(game,'right',12);U.wait(15);verify(true)
  assert(U.shot(game,dir..'/player-on-again.png'))
  ...
  for n,c in ipairs(cases)do
    ...
    option('pokemonModelSkin',c[3]);option('apo_pokemon_model_source',c[2])
    for _,key in ipairs(contexts)do option(key,c[1])end
    if n==3 then for _,key in ipairs({'apo_hd_pokemon_followers','apo_hd_pokemon_grass','apo_hd_pokemon_city','apo_hd_pokemon_wilds_towns'})do option(key,false)end
    else for _,key in ipairs({'apo_hd_pokemon_followers','apo_hd_pokemon_grass','apo_hd_pokemon_city','apo_hd_pokemon_wilds_towns'})do option(key,true)end end
    ...
    U.hold(game,'left',8);U.hold(game,'right',8);U.wait(25)
